import { useTranslation } from 'react-i18next';
import { useAppStore, type MotionPreference } from '@/store/useAppStore';
import { cn } from '@/lib/cn';

const options: MotionPreference[] = ['system', 'on', 'off'];

const selectClasses =
  'rounded-md border border-white/15 bg-brand-900 px-2 py-1.5 text-sm text-brand-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-300';

export function MotionToggle({ className }: { className?: string }) {
  const { t } = useTranslation();
  const motion = useAppStore((state) => state.settings.motion);
  const setMotion = useAppStore((state) => state.setMotionPreference);

  const labels: Record<MotionPreference, string> = {
    system: t('settings.motionSystem'),
    on: t('settings.motionOn'),
    off: t('settings.motionOff'),
  };

  return (
    <label className={cn('flex items-center gap-2', className)}>
      <span className="text-xs font-medium uppercase tracking-wide text-brand-200">
        {t('settings.motion')}
      </span>
      <select
        className={selectClasses}
        value={motion}
        onChange={(event) => setMotion(event.target.value as MotionPreference)}
      >
        {options.map((option) => (
          <option key={option} value={option}>
            {labels[option]}
          </option>
        ))}
      </select>
    </label>
  );
}
